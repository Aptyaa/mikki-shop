...
export function useSession(): void {
  ...
  // Старт клиента — отдельным эффектом и ровно один раз. Эффект входа ниже
  // перезапускается при смене токена (сервер отверг — вошли заново), а
  // `start()` просит полный экран: покупателя, который сам из него вышел,
  // затаскивало бы обратно на каждый такой перезапуск.
  useEffect(() => {
    start();
  }, []);
